...
type BadgeVariant = 'success' | 'warning' | 'danger' | 'info' | 'neutral' | 'processing';
...
const styles: Record<BadgeVariant, string> = {
  ...
  neutral: 'bg-slate-50 text-slate-700 ring-slate-600/20 dark:bg-slate-800 dark:text-slate-400 dark:ring-slate-400/20',
  processing: 'bg-violet-50 text-violet-700 ring-violet-600/20 dark:bg-violet-900/30 dark:text-violet-400 dark:ring-violet-400/20',
};
...
export default function StatusBadge({ label, variant, dot = false }: StatusBadgeProps) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset ${styles[variant]}`}>
