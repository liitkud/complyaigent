...
const stageOrder = ['extracting', 'comparing', 'compacting', 'categorizing', 'complete'] as const;
...
const mockStages: IngestStatus[] = [
  { task_id: '', status: 'extracting', progress_pct: 15, current_stage: 'Extracting text from document', eta_seconds: 10 },
  { task_id: '', status: 'comparing', progress_pct: 40, current_stage: 'Comparing against existing rules', eta_seconds: 8 },
  { task_id: '', status: 'compacting', progress_pct: 65, current_stage: 'Compacting & deduplicating', eta_seconds: 5 },
  { task_id: '', status: 'categorizing', progress_pct: 85, current_stage: 'Categorizing into rule buckets', eta_seconds: 3 },
  { task_id: '', status: 'complete', progress_pct: 100, current_stage: 'Ingestion complete', eta_seconds: null },
];
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      if (useMock) {
        ...
        mockIdxRef.current = Math.min(mockIdxRef.current + 1, mockStages.length - 1);
        return;
      }
      try {
    ...
    <div className="flex-1 overflow-y-auto">
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/80 px-6 py-4 backdrop-blur-sm dark:border-slate-700 dark:bg-slate-950/80">
        <div className="flex items-center gap-3">
          <Link href="/upload" className="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800">
            <ArrowLeft size={16} />
          </Link>
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white">Processing Status</h1>
            <p className="text-xs text-slate-400">
              Polling <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-[10px] dark:bg-slate-800">GET /ingest/{id}</code> every 2s
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-2xl space-y-6 p-6">
        {usingMock && (
          <div className="flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-900/10">
            ...
            <div>
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300">Backend unavailable — simulating progress</p>
              <p className="text-xs text-amber-600 dark:text-amber-400">
                Could not reach <code className="font-mono">GET /ingest/{'{id}'}</code>. Displaying mock pipeline stages for UI preview.
              </p>
            </div>
          </div>
        )}
        ...
        {status && (
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-blue-600 dark:text-blue-400">{status.progress_pct}%</span>
            </div>
        ...
        {/* Stepper */}
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
          <h3 className="mb-4 text-sm font-semibold text-slate-900 dark:text-white">Pipeline Stages</h3>
                ...
                <div key={stage} className="flex items-start gap-3">
                  ...
                  <div className="flex flex-col items-center">
                    ...
                    {i < stageOrder.length - 1 && (
                      <div className={`h-8 w-0.5 ${isDone ? 'bg-emerald-300 dark:bg-emerald-700' : 'bg-slate-200 dark:bg-slate-700'}`} />
                    )}
                  </div>
                  {/* Label */}
                  <div className="pt-1">
        ...
        {status?.status === 'complete' && (
          <div className="flex items-center gap-3 rounded-xl border border-emerald-200 bg-emerald-50/50 p-5 dark:border-emerald-800 dark:bg-emerald-900/10">
            ...
            <div>
              <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-300">Ingestion complete!</p>
